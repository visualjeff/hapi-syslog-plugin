...
internals.schema = Joi.object().keys({
    target: Joi.string().hostname().default('127.0.0.1'),
    syslogHostname: Joi.string().hostname().default(Os.hostname()),
    transport: Joi.number().integer().min(1).max(2).default(Transport.Udp),
    port: Joi.any().default(514), // A port can be both a number (integer) or a string,
    tcpTimeout: Joi.number().integer().default(10000),
    rfc3164: Joi.boolean().default(false),
    appName: Joi.string().default(process.title.substring(process.title.lastIndexOf('/') + 1, 48)),
    dateFormatter: Joi.func().default((date) => date.toISOString()),
    facility: Joi.number().integer().min(0).max(23).default(Facility.Local0),
    severity: Joi.number().integer().min(0).max(7).default(Severity.Informational)
});
        ...
        class Client extends EventEmitter {
                ...
                this.getTransport((error, transport) => {
                    ...
                    try {
                        if (options.transport === Transport.Tcp) {
                            ...
                            transport.send(fm, 0, fm.length, options.port, options.target, (error) => {
                                if (error) {
            ...
            getTransport(cb) {
                ...
                if (this.options.transport === Transport.Tcp) {
                    let opts;
                    if (this.options.port === '/tmp/echo.sock'){
                        opts = {
